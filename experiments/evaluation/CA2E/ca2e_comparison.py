...
for location in range(0, forecasts.shape[1]):
    print(f"\nEvaluating Location {location}")
    ...
    ca2e_ensemble, ca2e_weights = api.retrieve(z_t, z_h, y_h, k=100)
    ...
    a2e.io.plotting.plot_bias(
        y_true=y_t,
        ensemble_observations=ca2e_ensemble,
        member_weights=ca2e_weights,
        label="CA2E",
        ax=ax[0, 0]
    )
    a2e.io.plotting.plot_bias(
    ...
    save_path = directory / f"Location{location}" / f"{model_name}_evaluation" / f"{model_name}_{plot_type}-comparison.png"
