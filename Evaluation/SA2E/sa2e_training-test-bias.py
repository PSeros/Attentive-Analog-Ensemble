...
for location in range(82):
    ...
    # Retrieve top-k SA2E ensemble and corresponding weights for TRAINING data
    sa2e_ensemble_train, sa2e_weights_train = api.retrieve(z_t_train, z_h_train, y_h_train, k=100)

    # Compute bias for TRAINING data
    bias_train = A2E.metrics.computation.compute_bias(sa2e_ensemble_train, sa2e_weights_train, y_t_train)
    ...
    # Retrieve top-k SA2E ensemble and corresponding weights for TEST data
    sa2e_ensemble_test, sa2e_weights_test = api.retrieve(z_t_test, z_h_test, y_h_test, k=100)

    # Compute metrics for UNCORRECTED TEST data
    bias_test_uncorrected = A2E.metrics.computation.compute_bias(sa2e_ensemble_test, sa2e_weights_test, y_t_test)
    rmse_test_uncorrected = A2E.metrics.computation.compute_rmse(sa2e_ensemble_test, sa2e_weights_test, y_t_test)
    crps_test_uncorrected = -A2E.metrics.computation.compute_crps(sa2e_ensemble_test, sa2e_weights_test, y_t_test)
    scrps_test_uncorrected = -A2E.metrics.computation.compute_scrps(sa2e_ensemble_test, sa2e_weights_test, y_t_test)
    ...
    # ===== BIAS CORRECTION =====
    # Korrigiere ensemble mitglieder mit training bias
    sa2e_ensemble_test_corrected = sa2e_ensemble_test - bias_train_mean

    # Compute metrics for CORRECTED TEST data
    bias_test_corrected = A2E.metrics.computation.compute_bias(sa2e_ensemble_test_corrected, sa2e_weights_test, y_t_test)
    rmse_test_corrected = A2E.metrics.computation.compute_rmse(sa2e_ensemble_test_corrected, sa2e_weights_test, y_t_test)
    crps_test_corrected = -A2E.metrics.computation.compute_crps(sa2e_ensemble_test_corrected, sa2e_weights_test, y_t_test)
    scrps_test_corrected = -A2E.metrics.computation.compute_scrps(sa2e_ensemble_test_corrected, sa2e_weights_test, y_t_test)
...
print("\nSA2E Bias Correction Comparison Summary:")
