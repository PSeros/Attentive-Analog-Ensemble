...
combined_y_t = []
combined_a2e_ensemble = []
combined_a2e_weights = []
...
for location in range(forecasts.shape[1]):
    ...
    combined_a2e_ensemble.extend(a2e_ensemble.numpy())
    combined_a2e_weights.extend(a2e_weights.numpy())
...
A2E.io.plotting.rank_histogram(
    ...
    ensemble_observations=tf.constant(combined_a2e_ensemble),
    member_weights=tf.constant(combined_a2e_weights),
    handle_ties="uniform",
    ax=ax,
    title=model_type,
)
